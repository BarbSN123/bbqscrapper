            ...
            for i, (time_str, slot_id) in enumerate(slot_map.items(), start=1):
                ...
                for attempt in range(3):  # up to 3 retries
                    try:
                        ...
                        if not buffets:
                            failed_slots.append(
                                f"⚠️ No data for {branch_name} ({branch_id}) - {date_str} {time_str}"
                            )
                            break
                        ...
                        failed_slots.append(
                            f"❌ Error fetching {branch_name} ({branch_id}) - {date_str} {time_str}: {e}"
                        )
                        break

                if not success and len(failed_slots) == 0:
                    failed_slots.append(
                        f"❌ Unknown error for {branch_name} ({branch_id}) - {date_str} {time_str}"
                    )
